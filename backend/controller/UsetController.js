...
export const modifyTipStatus = async (request, response) => {
  try {
    console.log("modifyTipStatus", request.body);
    const newuser = await tip.updateOne(
      { _id: new ObjectId(request.body._id) },
      { $set: { status: request.body.status } }
    );
    // console.log("AftermodifyTipStatus", newuser);
    await newuser.save();
